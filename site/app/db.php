...
function seed_if_empty(PDO $pdo): void
{
    ...
    $password = password_hash((string) app_config()['seed_password'], PASSWORD_DEFAULT);
    $now = date('c');

    $ins = $pdo->prepare('INSERT INTO users (name, email, password, is_admin, created_at) VALUES (?, ?, ?, ?, ?)');
    $ins->execute(['Teqnowebs Admin', 'admin@teqnowebs.com', $password, 1, $now]);
    $ins->execute(['Teqnowebs Staff', 'staff@teqnowebs.com', $password, 0, $now]);
